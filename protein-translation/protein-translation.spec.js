...
describe('ProteinTranslation', () => {
  ...
  test('Invalid partial codon throws error', () => {
    expect(() => translate('UGGL')).toThrow(new Error('Invalid codon L'));
  });
  ...
  test('Invalid codon throws error', () => {
    expect(() => translate('LOL')).toThrow(new Error('Invalid codon LOL'));
  });
});
